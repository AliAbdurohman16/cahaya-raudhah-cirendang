...
<div class="db-info-wrap db-add-tour-wrap">
    <div class="row">
        <!-- Listings -->
        <div class="col-lg-12">
            <form action="{{ route('packages.update', $package) }}" class="form-horizontal" method="post" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                                ...
                                <div class="row">
                                    <div class="col-sm-3">
                                        <div class="text-center mb-4">
                                            <img src="{{ asset('storage/packages/' . $package->airline) }}" alt="image"class="img-thumbnail img-preview" width="100px">
                                        </div>
                                    </div>
                                    <div class="col-sm-9">
                ...
                <div class="dashboard-box">
                    @if($package->Hotels->isNotEmpty())
                        @foreach ($package->Hotels as $index => $hotel)
                            <h4>Hotel {{ $index + 1 }}</h4>
                            <div class="custom-field-wrap">
                                <div class="row">
                                    <div class="col-sm-2">
                                        <div class="form-group">
                                            <label>Bintang</label>
                                            <div class="start star-hotel-{{ $index + 1 }}">
                                                <i class="fa fa-star {{ $hotel->star >= 1 ? 'active' : '' }}" onclick="hotel{{ $index + 1 }}(1)"></i>
                                                <i class="fa fa-star {{ $hotel->star >= 2 ? 'active' : '' }}" onclick="hotel{{ $index + 1 }}(2)"></i>
                                                <i class="fa fa-star {{ $hotel->star >= 3 ? 'active' : '' }}" onclick="hotel{{ $index + 1 }}(3)"></i>
                                                <i class="fa fa-star {{ $hotel->star >= 4 ? 'active' : '' }}" onclick="hotel{{ $index + 1 }}(4)"></i>
                                                <i class="fa fa-star {{ $hotel->star >= 5 ? 'active' : '' }}" onclick="hotel{{ $index + 1 }}(5)"></i>
                                            </div>
                                            <input type="hidden" class="form-control @error('star_' . ($index + 1)) is-invalid @enderror" name="star_{{ $index + 1 }}" id="hotel{{ $index + 1 }}" value="{{ old('star_' . ($index + 1), $hotel->star) }}">
                                            @error('star_' . ($index + 1))
                                                <span class="invalid-feedback" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
                                    ...
                                    <div class="col-sm-5">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control @error('name_' . ($index + 1)) is-invalid @enderror" name="name_{{ $index + 1 }}" placeholder="Nama Hotel" value="{{ old('name_' . ($index + 1), $hotel->name) }}">
                                            @error('name_' . ($index + 1))
                                                <span class="invalid-feedback" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
                                    ...
                                    <div class="col-sm-5">
                                        <div class="form-group">
                                            <label>Kota</label>
                                            <input type="text" class="form-control @error('city_' . ($index + 1)) is-invalid @enderror" name="city_{{ $index + 1 }}" placeholder="Kota" value="{{ old('city_' . ($index + 1), $hotel->city) }}">
                                            @error('city_' . ($index + 1))
                                                <span class="invalid-feedback" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
